...
这套设计不是为了好看，是踩出来的：**纯 headless 的自动化很容易被网站风控盯上**，而"真实带界面的浏览器 + 关键时刻人来点两下"这个组合明显更扛。两点原因：

**1. headed 真浏览器的指纹更像真人。** 无头浏览器有一堆藏不住的破绽——`navigator.webdriver` 标志、缺插件、WebGL / 字体渲染和真机对不上、UA 里带 Headless。本项目跑的是**带界面的真 Chrome**（服务器上靠 xvfb 撑一块虚拟屏幕），用一份**长期养着的 profile**（cookie、历史、登录态都在），并关掉了 `AutomationControlled` 标志。在网站眼里，这更接近"一个老用户用自己电脑上的 Chrome"，而不是刚冒出来的无头爬虫。

**2. 撞上风控墙时，人可以在同一个会话里当场过关。** 滑块、图形验证码、短信验证、异常登录二次确认——纯脚本遇到这些要么直接跪，要么硬闯反而把当前会话标记成可疑。有了远程接管，你掏出手机在**同一个浏览器、同一个 IP、同一份指纹**上手动点过去，会话不断、不用重开；人手的操作节奏（真实的停顿、不规整的时机）也比脚本那种整齐划一的时序更难被行为风控挑出来。

> 一句实话别误会：接管时手机上的点按最终还是经 CDP 合成事件打进去的，不是操作系统级的真实触摸，所以指纹层面它并不等于"真人手指"。真正省事的地方在**节奏是人的** + **会话连续不重开**——这两点才是过行为风控的关键。所以正确姿势是：**能自动就自动，撞墙了人接一下**，而不是指望它把一切变透明。
